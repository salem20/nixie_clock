...
/**@brief Function for printing error info, which is called when an error has occurred.
 *
 * @param[in] error_code  Error code supplied to the handler.
 * @param[in] line_num    Line number where the handler is called.
 * @param[in] p_file_name Pointer to the file name.
 */
void app_error_print_info(uint32_t error_code, uint32_t line_num, const uint8_t * p_file_name);
#endif
...
static __INLINE void app_error_log(uint32_t id, uint32_t pc, uint32_t info)
{
    switch (id)
    {
        case NRF_FAULT_ID_SDK_ASSERT:
            ...
            {
                NRF_LOG_PRINTF(
		NRF_LOG_COLOR_WHITE "Line Number: %u\n",
		(unsigned int) ((assert_info_t *)(info))->line_num);NRF_LOG_PRINTF(
        ...
        case NRF_FAULT_ID_SDK_ERROR:
            NRF_LOG(
		NRF_LOG_COLOR_RED "\n*** APPLICATION ERROR *** \n" NRF_LOG_COLOR_WHITE);if (((error_info_t *)(info))->p_file_name)
            {
                NRF_LOG_PRINTF(
		"Line Number: %u\n", (unsigned int) ((error_info_t *)(info))->line_num);NRF_LOG_PRINTF(
...
#elif (NRF_LOG_USES_RTT == 1)
#define APP_ERROR_HANDLER(ERR_CODE)                                    \
    do                                                                 \
    {                                                                  \
    	app_error_print_info((ERR_CODE), __LINE__, (uint8_t*) __FILE__);  \
